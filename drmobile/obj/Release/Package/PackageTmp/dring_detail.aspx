...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <meta name="Description" content="Darry Ring官网提供<%=CurrentViewProduct.title%>求婚钻戒的定制服务，大量关于Darry Ring <%=CurrentViewProduct.title%>的钻戒图片以及款式可供挑选，想要购买一生只能购买一次的钻戒就来戴瑞珠宝官网。" />
    ...
    function addCart() {
        checkComplete = function(msg,data){
            if(msg == "true")
            {
                window.location = "IsBuyDarry.aspx?type=addcart&pid="+data.pid+"&did="+data.diamondId+"&handsize="+data.handsize+"&font="+data.font+"&price="+CurrentPrice+"&material="+data.caizhi;
            }
            else
            {
                w.Show(msg);
    ...
    function toBuy()
    {
        ...
        checkComplete = function(msg,data){
            if(msg == "true")
            {
                window.location = "IsBuyDarry.aspx?pid="+data.pid+"&did="+data.diamondId+"&handsize="+data.handsize+"&font="+data.font+"&price="+CurrentPrice+"&material="+data.caizhi;
            }
            else
            {
                w.Show(msg);
