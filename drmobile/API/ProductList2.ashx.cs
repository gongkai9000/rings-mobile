...
namespace drmobile.API
{
    ...
    /// ProductList 的摘要说明
    /// </summary>
    public class ProductList2 : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            {
                case "ctloves":
                    this.GetProductByDarryRign2(context);
                    break;
                case "jew":
                    this.GetProductByJew2(context);
                    break;
                case "jew2":
                    this.GetProductByJew2(context);
                    break;
        ...
        private void GetProductByJewbak2(HttpContext context)
        {
        ...
        private void GetProductByDarryRignbak2(HttpContext context)
        {
            List<view_product2> list = new List<view_product2>();
                    ...
                    if (p != null)
                    {
                        pstr +=
                            string.Format(
                                ...
                                p.id, p.imgurl,
                              StringHelper.SubString(p.title, 20) + " " + GetCtStr2(p.zct) + " " +
                                GetClolor2(p.zcolor), p.MemberPrice.Value.ToString("N0"), p.salecount);
        ...
        private void GetProductByJew2(HttpContext context)
        {
        ...
        private void GetProductByDarryRign2(HttpContext context)
        {
            List<view_product2> viewProduct2List = new List<view_product2>();
            ...
            if (list != null)
            {
                ...
                {
                    if (viewProduct2 != null)
                        s += string.Format("<li> <div class='mbdr_cp-img'><a href='ctring_detail.aspx?id={0}'><img alt='" + viewProduct2.title + "' src='{1}' width='140px' height='140px'> </a> </div><div class='mbdr_cp-word'><p class='hsp-height'>{2}</p><p><span class='fl'>￥{3}</span></p>", (object)viewProduct2.id, (object)viewProduct2.imgurl, (object)(StringHelper.SubString(viewProduct2.title, 20) + " " + this.GetCtStr2((object)viewProduct2.zct) + " " + this.GetClolor2((object)viewProduct2.zcolor)), (object)viewProduct2.MemberPrice.Value.ToString("N0"), (object)viewProduct2.salecount);
                }
            }
            if (list.Count == 0)
                context.Response.Write("s");
        ...
        /// <returns></returns>
        public string GetCtStr2(Object obj)
        {
            string str = "";
            if (obj != null)
            {
        ...
        /// <param name="obj"></param>
        /// <returns></returns>
        public string GetClolor2(Object obj)
        {
            string str = "";
            if (obj != null)
            {
